    ...
    FileList.prototype = {
        ...
        getModelForFile: function(fileName) {
            ...
            model.on('change', function(model) {
                ...
                $tr = self.updateRow(
                    $tr,
                    model.toJSON(), { updateSummary: true, silent: false, animate: true }
                );
        ...
        reloadCallback: function(status, result) {
            ...
            if (status === 500) {
                // Go home
                this.changeDirectory('/');
                OC.Notification.show(t('files', 'This directory is unavailable, please check the logs or contact the administrator'), { type: 'error' });
                return false;
            }
            ...
            if (status === 503) {
                ...
                if (this.getCurrentDirectory() !== '/') {
                    ...
                    OC.Notification.show(t('files', 'Storage is temporarily not available'), { type: 'error' });
                }
                return false;
            }
                ...
                self.filesClient.move(dir + fileName, targetPath + fileName)
                    ...
                    .fail(function(status, result) {
                        if (status === 412) {
                            ...
                            OC.Notification.show(t('files', 'Could not move "{file}", target exists', { file: fileName }, null, { escape: false }), { type: 'error' });
                        } else if (status === 423) {
                            OC.Notification.show(t('files', 'Could not move "{file}" because either the file or the target are locked.', { file: fileName, message: result.message }), { type: 'error' });
                        } else if (result != null && typeof result.message !== "undefined") {
                            OC.Notification.show(t('files', 'Could not move "{file}": {message}', { file: fileName, message: result.message }), { type: 'error' });
                        } else {
                            OC.Notification.show(t('files', 'Could not move "{file}"', { file: fileName }), { type: 'error' });
                        }
                    })
                    .always(function() {
                        ...
                        self.filesClient.move(OC.joinPaths(path, oldName), OC.joinPaths(path, newName))
                            ...
                            .fail(function(status) {
                                ...
                                if (status === 404) {
                                    ...
                                    OC.Notification.show(t('files', 'The file "{fileName}" is locked and can not be renamed.', { fileName: oldName }), { type: 'error' });
                                } else {
                                    // restore the item to its previous state
            ...
            this.filesClient.createDirectory(targetPath)
                .done(function() {
                    self.addAndFetchFileInfo(targetPath, '', { scrollTo: true }).then(function(status, data) {
                        ...
                        OC.Notification.show(t('files', 'Could not create folder "{dir}"', { dir: name }), { type: 'error' });
                    });
                })
                    ...
                    if (createStatus === 405) {
                        ...
                        self.addAndFetchFileInfo(targetPath, '', { scrollTo: true })
                            .done(function(status, data) {
                                OC.Notification.show(t('files', 'Could not create folder "{dir}" because it already exists', { dir: name }), { type: 'error' });
                ...
                self.filesClient.remove(dir + '/' + file)
                    ...
                    .fail(function(status) {
                        if (status === 404) {
                            ...
                            if (status === 423) {
                                OC.Notification.show(t('files', 'The file "{fileName}" is locked and cannot be deleted.', { fileName: file }), { type: 'error' });
                            } else {
                                OC.Notification.show(t('files', 'Error deleting file "{fileName}".', { fileName: file }), { type: 'error' });
                            }
